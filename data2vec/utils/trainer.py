...
def train_single_batch(net: nn.Module, data: torch.Tensor, mask: torch.Tensor, optimizer: optim.Optimizer, criterion: Callable, device: torch.device) -> Tuple[float, int]:
...
@torch.no_grad()
def evaluate(net: nn.Module, mask_generator, criterion: Callable, dataloader: DataLoader, device: torch.device) -> Tuple[float, float]:
    """Performs inference.
...
def train(net: nn.Module, mask_generator, optimizer: optim.Optimizer, criterion: Callable, trainloader: DataLoader, valloader: DataLoader, schedulers: dict, config: dict) -> None:
    """Trains model.
    ...
    for epoch in range(config["hparams"]["n_epochs"]):
        ...
        log_dict = {"epoch": epoch, "time_per_epoch": time.time() - t0, "train_target_var": running_target_var/n_batches, 
                    "train_prediction_var": running_prediction_var/n_batches, "avg_loss_per_ep": running_loss/len(trainloader.dataset)}
        log(log_dict, step, config)

        if not epoch % config["exp"]["val_freq"]:
            avg_val_loss, avg_val_target_var, avg_val_prediction_var = evaluate(net, mask_generator, criterion, valloader, device)
